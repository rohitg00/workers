...
const options = {
  ...
  request_format: zodToJsonSchema(PayloadSchema, { name: 'AlwaysAllowPayload' }),
...
export function registerAddAlwaysAllow(iii: ISdk): void {
  const handler: RemoteFunctionHandler<unknown, ApprovalSettings | MutationReply> = async (
    ...
    try {
      return ok(await addAlwaysAllow(iii, parsed.data.session_id, parsed.data.function_id));
    } catch (err) {
      return mutationError(err);
    }
  };

  iii.registerFunction('approval::add_always_allow', handler, options);
}
